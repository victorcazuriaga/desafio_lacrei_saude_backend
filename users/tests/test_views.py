...
class UserViewTest(APITestCase):
    ...
    def test_register_user(cls):
        response_register = cls.client.post(
            "/api/register", cls.user_data, format="json"
        )
    ...
    def test_register_user_exists(cls):
        ...
        response_register2 = cls.client.post(
            "/api/register", cls.user_data, format="json"
        )
    ...
    def test_register_user_without_password(cls):
        response_register = cls.client.post(
            "/api/register", cls.user_data["username"], format="json"
        )
        cls.assertEqual(response_register.status_code, 400)

    def test_register_user_without_username(cls):
        response_register = cls.client.post(
            "/api/register", cls.user_data["password"], format="json"
        )
        cls.assertEqual(response_register.status_code, 400)
